...
@section('content')
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <header
            class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-gray-100 mb-8">
            <div>
                <div class="flex items-center gap-3 flex-wrap">
                    <h1 class="text-2xl font-semibold tracking-tight text-gray-900">{{ $credential->title }}</h1>
                    @php
                ...
                <p class="text-sm text-gray-500 mt-1">
                    {{ \Carbon\Carbon::parse($credential->issue_date)->format('M j, Y g:i A') }} •
                    {{ ucfirst($credential->type) }} Category
                </p>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-50 bg-gray-50/30">
                        <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Credential Information</h2>
                    </div>

                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <p class="text-xs font-medium text-gray-400">Issuing Institution</p>
                            <p class="text-sm font-medium text-gray-900 mt-1">{{ $credential->issuer }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-400">Credential ID / Serial</p>
                            <p class="text-sm font-mono text-gray-900 mt-1">STFD-{{ $credential->id }}-CS</p>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-400">Issue Date</p>
                            <p class="text-sm font-medium text-gray-900 mt-1">
                                {{ \Carbon\Carbon::parse($credential->issue_date)->format('M j, Y') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-400">Expiration Date</p>
                            @if (!empty($credential->expiry_date))
                                <p class="text-sm font-medium text-gray-500 mt-1">
                                    {{ \Carbon\Carbon::parse($credential->expiry_date)->format('M j, Y') }}</p>
                            @else
                    ...
                    <div class="space-y-4">
                        ...
                        @elseif ($credential->status === 'pending')
                            <div class="bg-amber-50 border border-amber-100 rounded-xl p-4">
                                ...
                                <p class="text-xs text-amber-700 mt-0.5">
                                    This credential is waiting for verification.
                                </p>
                            </div>
                        @else
                        ...
                        <div class="divide-y divide-gray-50 text-xs">
                            ...
                            <div class="py-3 flex justify-between items-center">
                                ...
                                <span
                                    class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($credential->issue_date)->format('M j, Y g:i A') }}</span>
                            </div>
                            <div class="py-3 flex flex-col gap-1.5 justify-center">
                                <span class="text-gray-400">SHA-256 Checksum Hash</span>
                                <span
                                    class="font-mono bg-gray-50 p-2 rounded text-[10px] text-gray-600 truncate tracking-tight">
